package com.mealplanner.spoonacular.service;
...
import com.mealplanner.spoonacular.bindings.GenerateSingleDayMealPlanResponse;
import com.mealplanner.spoonacular.bindings.RecipeResponse;
		...
		return resp.subscribeOn(Schedulers.boundedElastic())
				...
				.flatMap(meal -> {
					...
					return recipeResponse.subscribeOn(Schedulers.boundedElastic())
							.log()
							.map(recipe -> new MealInfo(meal.getTitle(), meal.getReadyInMinutes(),recipe.getHealthScore(),
